...
void quickSort(int paper[][2], int begPos, int endPos) {
    if(begPos < endPos) {
        ...
        double povitUnit = paper[begPos][1]*1.0/paper[begPos][0];

        while(lo < hi) {
            while(lo < hi && paper[hi][1]*1.0/paper[hi][0] <= povitUnit) {
                hi--;
            }
            paper[lo][0] =paper[hi][0];
            paper[lo][1] =paper[hi][1];

            while(lo < hi && paper[lo][1]*1.0/paper[lo][0] >= povitUnit) {
                lo++;
            }
    ...
    *pMaxValue = 0;
    ...
    for(int i = 0; i < nPapers; i++) {
        if(consum + paper[i][0] <= nRemain) {
            ...
            *pMaxValue += paper[i][1]*1.0/paper[i][0]*(nRemain - consum);
            break;
        }
    }
